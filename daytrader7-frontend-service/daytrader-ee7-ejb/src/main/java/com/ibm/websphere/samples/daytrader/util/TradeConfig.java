...
public class TradeConfig implements KluInterface {
  ...
  public static String[] orderProcessingModeNames = { "Sync" };
  ...
  /*
   * Trade Caching Type parameters public static String[] cachingTypeNames = {
   * "DistributedMap", "No Caching" }; public static final int DISTRIBUTEDMAP = 0;
   * public static final int NO_CACHING = 1; public static int cachingType =
   * NO_CACHING; public static int distributedMapCacheSize = 100000;
   */
  ...
  public static String[][] webUI = {
      ...
      { "/welcomeImg.jsp", "/registerImg.jsp", "/portfolioImg.jsp", "/quoteImg.jsp", "/tradehomeImg.jsp",
          "/accountImg.jsp", "/orderImg.jsp", "/config.jsp", "/runStats.jsp", "/marketSummary.jsp" },
      // JSP Interface
  };
  ...
  public static void setConfigParam(String parm, String value) {
    ...
    if (parm.equalsIgnoreCase("runTimeMode")) {
      try {
        ...
        // >>rjm
        Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set runtimemode to " + value
            + "reverting to current value: " + runTimeModeNames[runTimeMode], e);
      ...
      try {
        ...
        Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set orderProcessingMode to "
            + value + "reverting to current value: " + orderProcessingModeNames[orderProcessingMode], e);
      } // If the value is bad, simply revert to current
      ...
      try {
        ...
        Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set accessMode to " + value
            + "reverting to current value: " + accessModeNames[accessMode], e);
      }
      ...
      try {
        ...
        Log.error("TradeConfig.setConfigParm(..): minor exception caught" + "trying to set WebInterface to " + value
            + "reverting to current value: " + webInterfaceNames[webInterface], e);
      ...
      try {
        ...
        Log.error("TradeConfig.setConfigParm(..): minor exception caught"
            + "Setting maxusers, error parsing string to int:" + value + "revering to current value: " + MAX_USERS, e);
      ...
      try {
        ...
        // >>rjm
        Log.error("TradeConfig.setConfigParm(...) minor exception caught"
            + "Setting max_quotes, error parsing string to int " + value + "reverting to current value: " + MAX_QUOTES,
            e);
        // <<rjm
      } // On error, revert to saved
       ...
       * else if (parm.equalsIgnoreCase("DistMapCacheSize")) { try {
       * distributedMapCacheSize = Integer.parseInt(value); } catch (Exception e) { //
       * >>rjm Log.error("TradeConfig.setConfigParm(...) minor exception caught" +
       * "Setting distributedMapCacheSize, error parsing string" + value +
       * "reverting to current value: " + distributedMapCacheSize, e); // <<rjm } //
       * On error, revert to saved }
       */
  }
