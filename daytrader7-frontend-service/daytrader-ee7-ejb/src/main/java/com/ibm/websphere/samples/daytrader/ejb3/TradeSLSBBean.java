...
public class TradeSLSBBean implements TradeSLSBLocal {
   ...
   * javax.annotation.Resource.AuthenticationType.APPLICATION) private
   ...
   * 
   * @Resource(lookup = "java:/jms/TradeStreamerTopic") private Topic
   * tradeStreamerTopic;
   * 
   * @Resource(lookup = "java:/jms/TradeBrokerQueue") private Queue
   * tradeBrokerQueue;
   */
  ...
  public MarketSummaryDataBean getMarketSummary() {
    ...
    //TODO: Add remote call to get MarketSummary?
    return null;
  ...
  public OrderDataBean completeOrder(Integer orderID, boolean twoPhase) throws Exception {
    ...
    if (order.isSell()) {
       ...
       */
      if (holding == null) {
        // Log.error("TradeSLSBBean:completeOrder -- Unable to sell order " +
        // order.getOrderID() + " holding already sold");
        order.cancel();
        throw new EJBException(
            "TradeSLSBBean:completeOrder -- Unable to sell order " + order.getOrderID() + " holding already sold");
      } else {
  ...
  public Collection<HoldingDataBean> getHoldings(String userID) {
    ...
    criteriaQuery.where(criteriaBuilder.equal(holdings.get("account").get("profile").get("userID"),
        criteriaBuilder.parameter(String.class, "p_userid")));
  ...
  private OrderDataBean createOrder(AccountDataBean account, QuoteDataBean quote, HoldingDataBean holding,
    ...
    if (Log.doTrace()) {
      Log.trace("TradeSLSBBean:createOrder(orderID=" + " account=" + ((account == null) ? null : account.getAccountID())
          + " quote=" + ((quote == null) ? null : quote.getSymbol()) + " orderType=" + orderType + " quantity="
          + quantity);
    }
    try {
      order = new OrderDataBean(orderType, "open", new Timestamp(System.currentTimeMillis()), null, quantity,
          quote.getPrice().setScale(FinancialUtils.SCALE, FinancialUtils.ROUND), TradeConfig.getOrderFee(orderType),
          account, quote, holding);
