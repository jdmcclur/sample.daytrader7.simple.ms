...
public class TradeConfigServlet extends HttpServlet {
  ...
  void doConfigDisplay(HttpServletRequest req, HttpServletResponse resp, String results) throws Exception {
    ...
    getServletConfig().getServletContext().getRequestDispatcher(TradeConfig.getPage(TradeConfig.CONFIG_PAGE))
        .include(req, resp);
  }
  ...
  void doResetTrade(HttpServletRequest req, HttpServletResponse resp, String results) throws Exception {
    ...
    getServletConfig().getServletContext().getRequestDispatcher(TradeConfig.getPage(TradeConfig.STATS_PAGE))
        .include(req, resp);

  }

  /**
   * Update Trade runtime configuration paramaters Creation date: (2/8/2000
   * 3:44:24 PM)
   */
  void doConfigUpdate(HttpServletRequest req, HttpServletResponse resp) throws Exception {
    ...
    if (runTimeModeStr != null) {
      try {
        ...
        // >>rjm
        Log.error(e, "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
            "trying to set runtimemode to " + runTimeModeStr, "reverting to current value");
    ...
    if (orderProcessingModeStr != null) {
      try {
        ...
        // >>rjm
        Log.error(e, "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
            "trying to set orderProcessing to " + orderProcessingModeStr, "reverting to current value");
    ...
    if (webInterfaceStr != null) {
      try {
        ...
        Log.error(e, "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
            "trying to set WebInterface to " + webInterfaceStr, "reverting to current value");
    ...
    /*
     * String cachingTypeStr = req.getParameter("CachingType"); if (cachingTypeStr
     * != null) { try { int i = Integer.parseInt(cachingTypeStr); if ((i >= 0) && (i
     * < TradeConfig.cachingTypeNames.length)) //Input validation
     * TradeConfig.setCachingType(i); } catch (Exception e) { Log.error(e,
     * "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
     * "trying to set CachingType to " + cachingTypeStr,
     * "reverting to current value"); } // If the value is bad, simply revert to
     * current } currentConfigStr += "\t\tCachingType:\t\t\t" +
     * TradeConfig.cachingTypeNames[TradeConfig.cachingType] + "\n";
     * 
     * String distMapCacheSize = req.getParameter("DistMapCacheSize"); if
     * ((distMapCacheSize != null) && (distMapCacheSize.length() > 0)) { try {
     * TradeConfig.setDistributedMapCacheSize(Integer.parseInt(distMapCacheSize)); }
     * catch (Exception e) { Log.error(e,
     * "TradeConfigServlet: minor exception caught",
     * "trying to set DistributedMapCacheSize, error on parsing int " +
     * distMapCacheSize, "reverting to current value " +
     * TradeConfig.getPrimIterations());
     * 
     * } } currentConfigStr += "\t\tDMap Cache Size:\t\t" +
     * TradeConfig.getDistributedMapCacheSize() + "\n";
     */
    ...
    if ((parm != null) && (parm.length() > 0)) {
      try {
        ...
        // >>rjm
        Log.error(e, "TradeConfigServlet: minor exception caught",
    ...
    if ((parm != null) && (parm.length() > 0)) {
      try {
        ...
        Log.error(e, "TradeConfigServlet: minor exception caught",
            "trying to set percentSentToWebSocket, error on parsing int " + parm,
  ...
  public void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    try {
      ...
      if (action == null) {
        ...
        resp.setContentType("text/html");
        TradeDbUtils.populateDB(resp.getWriter());
        ...
        try {
          dbProductName = TradeDbUtils.checkDBProductName();
        } catch (Exception e) {
        ...
        try {
          ...
          if (dbProductName.startsWith("DB2/")) {// if db is DB2
            ...
            ddlFile = "/dbscripts/derby/Table.ddl";
            resp.getWriter()
                .println("<BR>TradeBuildDB: **** This Database is unsupported/untested use at your own risk ****</BR>");
          }

          resp.getWriter()
              .println("<BR>TradeBuildDB: **** The DDL file at path <I>" + ddlFile + "</I> will be used ****</BR>");
          resp.getWriter().flush();
        } catch (Exception e) {
          Log.error(e, "TradeBuildDB: Unable to locate DDL file for the specified database");
          resp.getWriter()
              .println("<BR>TradeBuildDB: **** Unable to locate DDL file for the specified database ****</BR></BODY>");
          return;
        }

        TradeDbUtils.buildDB(resp.getWriter(), getServletContext().getResourceAsStream(ddlFile));
