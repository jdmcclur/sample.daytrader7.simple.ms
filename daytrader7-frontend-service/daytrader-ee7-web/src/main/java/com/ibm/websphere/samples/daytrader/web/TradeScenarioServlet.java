  ...
  public void performTask(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    if ((scenarioAction != null) && (scenarioAction.length() >= 1)) {
      ...
      if (action == 'n') { // null;
        try {
          ...
          Log.error("trade_client.TradeScenarioServlet.service(...)"
    ...
    switch (action) {
      ...
      case 'u': // update account profile
        ...
        dispPath = tasPathPrefix + "update_profile&fullname=" + fullName + "&password=" + password + "&cpassword="
            + password + "&address=" + address + "&email=" + email + "&creditcard=" + creditcard;
        ctx.getRequestDispatcher(dispPath).include(req, resp);
        break;
      case 'h': // home
      ...
      case 'r': // register
        ...
        dispPath = tasPathPrefix + "register&Full Name=" + fullName + "&snail mail=" + smail + "&email=" + email
            + "&user id=" + userID + "&passwd=" + passwd + "&confirm passwd=" + passwd + "&money=" + money
      ...
      case 's': // sell
        ...
        if (numHoldings > 0) {
          ...
          if (Log.doTrace()) {
            Log.trace("TradeScenario: No holding to sell -switch to buy -- userID = " + userID + "  Collection count = "
                + numHoldings);
          }
         ...
         * When a users portfolio is reduced to 0 holdings, a buy is requested instead
         * of a sell. This throws off the buy/sell mix by 1. This results in unwanted
         * holding table growth To fix this we increment a sell deficit counter to
         * maintain the correct ratio in getScenarioAction The 'z' action from
         * getScenario denotes that this is a sell action that was switched from a buy
