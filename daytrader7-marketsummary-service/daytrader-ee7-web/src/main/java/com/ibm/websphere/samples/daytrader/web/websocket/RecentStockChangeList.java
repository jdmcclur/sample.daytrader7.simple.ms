  ...
  public static JsonObject stockChangesInJSON() {
    ...
    try {
      ...
      for (Iterator<Message> iterator = temp.iterator(); iterator.hasNext();) {
        ...
        jsonObjectBuilder.add("change" + i + "_stock", message.getStringProperty("symbol"));
        jsonObjectBuilder.add("change" + i + "_price", "$" + message.getStringProperty("price"));

        BigDecimal change = new BigDecimal(message.getStringProperty("price"))
            .subtract(new BigDecimal(message.getStringProperty("oldPrice")));
        change.setScale(2, RoundingMode.HALF_UP);

        jsonObjectBuilder.add("change" + i + "_change", change.toString());
