...
public class DBUtil {
  ...
  public void createDB() throws Exception {
    ...
    System.out.println(
        "DBUtil: Building DayTrader Holding Database... This operation will take several minutes. Please wait...");
    ...
    if (ddlFile != null) {
      ...
      System.out.println("DBUtil: **** Dropping and Recreating the Holding tables... ****");
      try {
      ...
      if (!success) {
        System.out
            .println("DBUtil: **** Unable to drop and recreate DayTrader Db Holding Tables, please check for database "
  ...
  public void resetTrade(boolean deleteAll) throws Exception {
    ...
    try {
      ...
      stmt = getStatement(conn, "delete from holdingejb where accountid is null");
      stmt.executeUpdate();
      stmt.close();

      stmt = getStatement(conn, "delete from holdingejb where accountid like 'ru:%'");
      stmt.executeUpdate();
      stmt.close();
